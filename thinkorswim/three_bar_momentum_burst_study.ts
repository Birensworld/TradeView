...
#
# The pattern is: the last 3 bars are all green (close > open) with each
# closing higher than the one before it, gaining at least minGainPct%
# (default 3) from the 1st of those 3 to the 3rd. If a streak runs 4 or 5
# greens in a row, the arrow only appears on the LAST green day of that
# streak (the 3rd soldier), not on every qualifying day within it -
# nextBarBreaksStreak below enforces that by requiring the next bar to NOT be
# green (or there being no next bar yet, i.e. today).
#
...
# ---- Each soldier must close higher than the one before it ----
def ascendingCloses = close > close[1] and close[1] > close[2];
...
def signal = last3Green and ascendingCloses and bigGain and nextBarBreaksStreak;
